...
class Backend:
    ...
    def backend_ba(self, t_start, t_end, steps, graph, nms, radius, thresh, max_factors, t_start_loop=None, loop=False, motion_only=False, enable_wq=True, enable_update_uncer=False, enable_udba=False, visualization_stage=False, save_edges_weights=False):
        ...
        graph.update_lowmem(
            ...
            save_edges_weights=save_edges_weights,
        )
    ...
    @torch.no_grad()
    def dense_ba(self, steps=6, enable_wq=True, enable_update_uncer=False, enable_udba=False, save_edges_weights=False):
        t_start = 0
    ...
    def loop_ba(self, t_start, t_end, steps=6, motion_only=False, local_graph=None, enable_wq=True):
        ...
        window = self.backend_loop_window   # 25
        max_factors = 8 * window
        nms = self.backend_loop_nms         # 10
        ...
        n_edges = self.backend_ba(t_start, t_end, steps, graph, nms, radius, thresh, 
                          left_factors, t_start_loop=t_start_loop, loop=True, 
                          motion_only=motion_only, enable_wq=enable_wq,
                          enable_update_uncer=True, enable_udba=True, visualization_stage=True)
        del graph
        torch.cuda.empty_cache()
