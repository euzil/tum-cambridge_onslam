...
from AB_1.src.gui.gl_render.render_ogl import OpenGLRenderer
from AB_1.src.gui.gui_utils import (
    GaussianPacket,
    Packet_vis2main,
    create_frustum,
    cv_gl,
...
class SLAM_GUI:
    ...
    def add_camera(self, camera, name, color=[0, 1, 0], gt=False, size=0.2):
        W2C = (
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.current_frame is not None:
            frustum = self.add_camera(
                gaussian_packet.current_frame, name="current", color=[0, 1, 0], size=0.2
            )
        ...
        if gaussian_packet.keyframes is not None:
            for iid,keyframe in enumerate(gaussian_packet.keyframes):
                if gaussian_packet.keyframe_colors is not None:
                    color = gaussian_packet.keyframe_colors[iid]
                else:
                    color = [0, 0, 1]
                name = "keyframe_{}".format(keyframe.uid)
                frustum = self.add_camera(keyframe, name=name, color=color)

        if gaussian_packet.full_traj is not None:
            trajectory_points = np.array(gaussian_packet.full_traj, dtype=np.float64)

            # Create a LineSet from trajectory points
            trajectory_line = o3d.geometry.LineSet()

            # Set the points after converting to numpy array
            ...
            # Optional: Set color for the trajectory (e.g., red)
            ...
            try:
                ...
                pass  # Geometry didn't exist
            self.widget3d.scene.add_geometry("trajectory", trajectory_line, self.lit)

            
        if gaussian_packet.full_traj_gt is not None:
            trajectory_points = np.array(gaussian_packet.full_traj_gt, dtype=np.float64)
            ...
            # Optional: Set color for the trajectory (e.g., red)
            colors = [[0, 0, 1] for _ in range(len(lines))]  # Red color
            trajectory_line.colors = o3d.utility.Vector3dVector(colors)

            # Add to scene
            try:
                self.widget3d.scene.remove_geometry("trajectory_gt")
            except Exception:
                pass  # Geometry didn't exist
            self.widget3d.scene.add_geometry("trajectory_gt", trajectory_line, self.lit)
