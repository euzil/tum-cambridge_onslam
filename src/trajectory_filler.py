...
from AB_1.src.utils.Printer import FontColor
...
class PoseTrajectoryFiller:
    ...
    def __fill(self, timestamps, images, depths, intrinsics, dino_features):
        ...
        if self.uncertainty_aware:
            # interpolate the uncertainty mask
        ...
        for _ in range(12):
            graph.update(N, N+M, motion_only=True, enable_update_uncer=False, enable_udba=True)
    ...
    def __call__(self, image_stream:BaseDataset):
        ...
        for (timestamp, image, _ , _)  in tqdm(image_stream):
            ...
            if self.uncertainty_aware and self.cfg['traj_filler']['use_dino_features']:
