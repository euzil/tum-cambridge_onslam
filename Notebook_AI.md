...
动态预测（dynamic_prediction）模块旨在对场景中的动态物体进行检测、跟踪和未来状态预测，提升在动态环境下的 SLAM 系统鲁棒性和精度。

**主要内容与功能：**

- **d4rt_bridge.py**：动态预测与主 SLAM 系统的数据桥梁，负责数据流的对接与转换。
- **evaluator.py**：用于评估动态预测模块的性能，包括预测准确率、ADE/FDE 等指标。
- **point_predictor.py**：实现对动态点云/特征点的运动预测（如匀速模型、Kalman Filter）。
- **sliding_window.py**：滑动窗口机制，聚合多帧信息进行动态物体检测与预测。
- **visualizer_4d.py**：可视化动态预测结果，支持空间（3D）和时间（1D）上的动态展示。

**典型流程：**
1. 数据输入：从主 SLAM 系统或数据集获取 RGB-D 图像、点云、位姿等信息。
2. 动态检测：利用滑动窗口等方法，检测场景中的动态物体或动态点。
3. 运动预测：对检测到的动态目标进行轨迹预测，输出未来帧的状态估计。
4. 结果评估与可视化：通过 evaluator.py 评估预测效果，并用 visualizer_4d.py 进行可视化展示。
5. 与主流程集成：通过 d4rt_bridge.py 将动态预测结果反馈给 SLAM 主流程，实现动态场景下的鲁棒建图与定位。

**作用与意义：**
- 有效区分静态与动态场景，提升 SLAM 在动态环境下的表现。
- 预测动态物体的运动趋势，为后续的路径规划、避障等任务提供支持。
- 通过可视化和评估，便于算法调优和系统分析。
